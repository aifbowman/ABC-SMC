...
function APMC(N,expd,models,rho,;names=Vector[[string("parameter",i) for i in 1:length(models[m])] for m in 1:length(models)],prop=0.5,paccmin=0.02,n=2,wks=4-length(workers()))
  addprocs(wks)
  ...
  temp=temp[:,find(temp[maximum(np)+2,:].<=epsilon[i])]
  temp=temp[:,1:s]
  for j in 1:lm
...
function APMC_KDE(N,expd,models,rho,;names=Vector[[string("parameter",i) for i in 1:length(models[m])] for m in 1:length(models)],prop=0.5,paccmin=0.02,wks=4-length(workers()))
  addprocs(wks)
  @everywhere include(joinpath(homedir(),"Box Sync","Dropbox","FishProjectR","julia","juliarc.jl"))
  i=1
  lm=length(models)
...
function APMC_KDE_adpt(N,expd,models,rho,;names=Vector[[string("parameter",i) for i in 1:length(models[m])] for m in 1:length(models)],prop=0.5,paccmin=0.02,ecv=0.1,B=10,wks=4-length(workers()))
  ...
  while maximum(pacc[:,i])>paccmin
    ...
    temp2=@parallel hcat for j in 1:Int(ceil(N2/prop)-N1)
      cont_KDE(models,pts,wts,expd,np,i,ker,rho)
    end
    its=vcat(its,sum(temp2[size(temp2)[1],:]))
    temp=hcat(temp,temp2)
    ...
    for j in 1:lm
      pts[j,i]=temp[2:(np[j]+1),find(temp[1,:].==j)]
      if size(pts[j,i])[2]>0
        keep=inds[find(reshape(temp[1,:].==j,N2))].<=N1
        wts[j,i]= collect(@parallel vcat for k in 1:length(keep)
          if !keep[k]
            pdf(models[j],(pts[j,i][:,k]))/pdf(ker[j,i-1],pts[j,i][:,k])
          else
    ...
    sizes=floor.(collect(linspace(N2/3,N2*2,10)))
    ecvs=Array{Float64}(length(sizes))
    for j in 1:length(sizes)
        neff=sizes[j]
        bs=(4/(neff*(np[1]+2)))^(1/(np[1]+4))
...
for j in 1:length(sizes)
    ...
    cvs=Array{Array}(lm)
    for n in 1:lm
    cvs[n]= @parallel vcat for k in 1:size(pts[n])[2]
